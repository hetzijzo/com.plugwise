...
class Anna extends EventEmitter {
  ...
  _getMode(callback) {
    ...
    // Make API request
    request({
      url: this._createAPIUrl() + '/rules',
      timeout: 2000,
      method: 'GET',
      headers: { 'Content-Type': 'text/xml' }
    }, function (error, response, body) {
      console.log(`_getMode() -> this.status = ${this.status}`)
      ...
      if (error) {
        ...
        if (doc) {
          ...
          if(!doc.rule) return callback(this.mode);
          if (!Array.isArray(doc.rule)) doc.rule = [doc.rule];

          // Loop over all elements in the XML document
					var rule_data = doc.rule.filter(function (x) {
						return x.active === 'true'
					})[0];

					if (rule_data !== 'undefined' && rule_data.name !== 'undefined') {
						return callback(this._parseMode(rule_data.name));
					}
				}
				else {
        	callback(this.mode);
  ...
  _parseMode(ruleName) {
    ...
    this._getMode((mode) => {
      if (mode) {
        ...
        const newValue = parseFloat(parseFloat(mode).toFixed(1));

        // If value differs from previous
        if (!isNaN(newValue) && this.mode !== newValue && this.mode !== undefined) {

          console.log("Anna: mode changed to " + newValue);

          // Emit event
          this.emit("mode", this, newValue);
        }

        // Update value
        if (!isNaN(newValue)) this.mode = newValue;
      }
    });
	}
